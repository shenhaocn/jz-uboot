...
/*
	...
	printf("-----------------------------------------\n");
	printf("rBANKCON1= 0x%x\n",rBANKCON1);
	printf("-----------------------------------------\n");
	
	printf("-----------------------------------------\n");
	printf("rBANKCON4= 0x%x\n",rBANKCON4);
	printf("-----------------------------------------\n");
*/
	...
	/*	printf("MAC: %02x:%02x:%02x:%02x:%02x:%02x(%s)\n", bd->bi_enetaddr[0],
	       ...
	       bd->bi_enetaddr[4], bd->bi_enetaddr[5],mac_src);*/
	...
	for (i = 0; i < 6; i++)
		DM9000_iow(DM9000_PAR +i, bd->bi_enetaddr[i]);
	for (i = 0; i < 8; i++)
		DM9000_iow(DM9000_MAR+i, 0xff);

	for (i = 0; i < 6; i++){
		 bd->bi_enetaddr[i]=0;
		 bd->bi_enetaddr[i]=DM9000_ior(DM9000_PAR+i);
	}
...
{	
	...
	while (1) 
	{		
		status = DM9000_ior(DM9000_EPCR);		
		if ((status & EPCR_ERRE) == 0)			
			break;		
		if (timeout-- < 0)
		{			
			printf("timeout waiting EEPROM\n");			
			break;		
		}	
	}	
	return 0;
...
{	
	...
	DM9000_iow(DM9000_EPCR, EPCR_WEP | EPCR_ERPRW);
	udelay(200);		/* same shit */	
